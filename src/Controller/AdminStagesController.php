...
class AdminStagesController extends AbstractController
{
     ...
     * @return Response
     */
    public function EighthFinals(TeamsRepository $teamsRepo, GroupsRepository $groupsRepo, MatchsRepository $matchsRepo, StadesRepository $stadesRepo, DatesRepository $datesRepo, StagesRepository $stagesRepo, EntityManagerInterface $manager, SortService $sortService)
    {
        if(count($matchsRepo->findGroupMatchsPlayed()) == 36 && count($datesRepo->findFreeDates()) >= 8 && count($stadesRepo->findAll()) >= 12){
            ...
            $this->addFlash(
                "danger",
                "Attention toutes les conditions ne sont pas réunies pour commencer cette phase !"
            );
            return $this->redirectToRoute('admin_stages_index');
        }
    }
    ...
    {   
        if(count($matchsRepo->findStageMatchsPlayed(1)) == 8 && count($datesRepo->findFreeDates()) >= 4 && count($stadesRepo->findAll()) >= 12){
    ...
    {   
        if(count($matchsRepo->findStageMatchsPlayed(2)) == 4 && count($datesRepo->findFreeDates()) >= 2 && count($stadesRepo->findAll()) >= 12){
    ...
    {   
        if(count($matchsRepo->findStageMatchsPlayed(3)) == 2 && count($datesRepo->findFreeDates()) >= 1 && count($stadesRepo->findAll()) >= 12){
